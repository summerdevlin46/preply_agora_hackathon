...
### 1. Vision model for worksheet parsing (not traditional OCR)
...
### 2. Single LLM call generates all four exercise-mode prompts
...
### 3. Narrative prompt format for the Anam avatar (not template sections)
...
### 4. SQLite for prompt persistence (chat_id keyed)
...
### 5. Regeneration uses teacher feedback (not random retry)
